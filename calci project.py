...
root = Tk()
root.title("Cavy's Simple Calculator")
...
def ScitechzCalc(source, side):
    storeObj = Frame (source, borderwidth=4, bd=4, bg="grey5")
    storeObj.pack(side=side, expand=YES, fill=BOTH)
    return storeObj

def button(source, side, text, command=None):
    storeObj = Button(source, bg="black", fg="white", text=text, command=command)
    storeObj.pack(side=side, expand=YES, fill=BOTH)
    return storeObj
...
class app(Frame):
    def __init__(self):
        ...
        Entry(self, relief=RIDGE,
                textvariable=display,justify='right',bd=26,fg="white",bg="black").pack(side=TOP, expand=YES,
                        fill=BOTH)
